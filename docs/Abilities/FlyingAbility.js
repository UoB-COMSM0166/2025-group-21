...
class FlyingAbility {
    ...
    // Flying applies upwards velocity boost
    applyUpwardForce() {
    ...
    // Recharges the flight charge
    charge() {
        if (this.chargeLeft < this.maxCharge) {
            if (!domains.game.pause.active) { this.chargeLeft += (this.level+1) / 2; }
            this.displayChargeLeft();
        }
    }

    // Displays the flight charge on the screen
    displayChargeLeft() {
        push();
            let d = width*0.003;
            fill(0);
...
if (typeof module !== 'undefined') { module.exports = FlyingAbility; }